...
/* ─── Normalized portalRole → workspace portal key ─────── */
const PORTAL_ROLE_TO_PORTAL = {
  user: "user-dashboard",
  professional: "user-dashboard",
  culinary: "user-dashboard",
  admin: "admin-desk",
  nutritionist: "clinical-desk",
  pharmacist: "pharmacist-desk",
};

// Reads the profile that SignInPage / SignUpPage store after a successful
// login/registration (GET /auth/profile) and resolves it to a portal key.
function resolvePortalFromStoredUser(fallbackPortal) {
  try {
    const raw = localStorage.getItem("dwm_user");
    if (!raw) return fallbackPortal || "user-dashboard";
    const user = JSON.parse(raw);
    return PORTAL_ROLE_TO_PORTAL[user.portalRole] || fallbackPortal || "user-dashboard";
  } catch {
    return fallbackPortal || "user-dashboard";
  }
}
...
export default function LandingPage() {
  ...
  // Always returns to the very top of the landing page — the canonical "Home".
  const goHome = () => {
    setMenuOpen(false);
    ...
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: "#f5f0e8", fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif" }}>
      ...
      <header className="w-full border-b border-gray-200 bg-[#f5f0e8]/95 px-6 sm:px-10 py-3.5 flex items-center justify-between sticky top-0 z-40 backdrop-blur relative" style={{ animation: "headerIn 0.4s ease both" }}>
        ...
        <nav className="hidden lg:flex items-center gap-7">
          <button
            onClick={goHome}
            className="nav-underline text-sm font-semibold text-gray-500 hover:text-[#1a3d2e] transition-colors"
          >
            Home
          </button>
          {NAV_LINKS.map(link => (
        ...
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-2.5">
            <button onClick={() => { setPendingPortal("user-dashboard"); setAuthPage("signin"); }} className="text-sm font-bold text-gray-900 hover:text-[#1a3d2e] transition-colors px-1">
              Sign In
            </button>
            <button
              onClick={() => { setPendingPortal("user-dashboard"); setAuthPage("signup"); }}
              className="text-sm font-bold text-white bg-[#1a3d2e] hover:bg-[#123024] transition-colors px-4 py-2 rounded-full"
            >
              Sign Up
            </button>
          </div>
        ...
        {menuOpen && (
          <div className="absolute top-full left-0 right-0 bg-[#f5f0e8] border-b border-gray-200 lg:hidden shadow-lg max-h-[80vh] overflow-y-auto" style={{ animation: "slideUp 0.2s ease both" }}>
            <div className="flex flex-col px-6 py-3">
              ...
              <button
                onClick={() => { setPendingPortal("user-dashboard"); setAuthPage("signup"); setMenuOpen(false); }}
                className="mt-3 py-2.5 rounded-xl text-sm font-bold text-white text-center"
                style={{ backgroundColor: "#1a3d2e" }}
              >
                Sign Up
              </button>
              <button
                onClick={() => { setPendingPortal("user-dashboard"); setAuthPage("signin"); setMenuOpen(false); }}
                className="mt-2 mb-1 py-2.5 rounded-xl text-sm font-bold text-[#1a3d2e] text-center border border-[#1a3d2e]/25"
              >
                Sign In
              </button>
            </div>
