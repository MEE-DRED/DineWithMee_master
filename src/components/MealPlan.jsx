...
const SmallMealCard = ({ id, badge, img, title, desc, tags, btnLabel, icon, onAction }) => (
  <div className="bg-white rounded-2xl overflow-hidden shadow-sm flex flex-col">
    <div className="relative h-44 overflow-hidden">
      <img src={img || "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=500"} alt={title} className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" />
      <MealBadge label={badge} />
    </div>
    <div className="p-5 flex flex-col flex-1">
      <h3 className="font-black text-[#1A1A1A] text-lg mb-1">{title}</h3>
      <p className="text-gray-400 text-sm mb-3 leading-relaxed line-clamp-2">{desc}</p>
      ...
      <div className="flex items-center gap-3 mt-auto">
        <button 
          onClick={() => onAction(id)}
          className="flex-1 bg-[#1E3D30] text-white text-sm font-bold py-3 rounded-xl hover:bg-[#162e23] active:scale-95 transition-all duration-150"
        >
          {btnLabel}
        </button>
        <button className="w-11 h-11 rounded-xl border border-gray-200 flex items-center justify-center hover:bg-gray-50 active:scale-95 transition-all">
          <Icon d={Icons[icon]} size={16} className="text-gray-500" />
        </button>
      </div>
    </div>
...
export default function DineWithMee() {
  ...
  // API Bound Dashboard States
  const [featuredMeal, setFeaturedMeal] = useState(null);
  const [complementaryMeals, setComplementaryMeals] = useState([]);
  const [recommendedSwaps, setRecommendedSwaps] = useState([]);
  const [nutritionalSummary, setNutritionalSummary] = useState({ current: 1320, target: 1900 });
  const [loading, setLoading] = useState(true);

  const progressPercentage = Math.min(Math.round((nutritionalSummary.current / nutritionalSummary.target) * 100), 100) || 0;

  // ── READ: Fetch items on mount using discovered Swagger routes ──
  useEffect(() => {
    async function loadDashboardData() {
      try {
        ...
        console.error("Network interface connection error. Using client side state layers.", err);
  ...
  const applyVisualFallbackData = () => {
    setFeaturedMeal({
      _id: "feat-100",
      badge: "LUNCH (TODAY)",
      img: "https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=800&q=80",
      title: "Roasted Harvest Quinoa Bowl",
      desc: "A vibrant combination of organic tri-color quinoa, maple-roasted butternut squash, massaged curly kale, and a creamy lemon-tahini dressing drizzle.",
      tags: ["520 KCAL", "LOW GLYCEMIC"]
    });
    setComplementaryMeals([
      { _id: "comp-1", badge: "BREAKFAST", img: "https://images.unsplash.com/photo-1525351484163-7529414344d8?w=500&q=80", title: "Avocado Toast & Egg", desc: "Thick sourdough slice topped with crushed Haas avocado, sea salt microgreens.", tags: ["340 KCAL", "HIGH PROTEIN"], btnLabel: "Log Breakfast", icon: "plus" },
      { _id: "comp-2", badge: "DINNER", img: "https://images.unsplash.com/photo-1467003909585-2f8a72700288?w=500&q=80", title: "Lemon Herb Salmon", desc: "Wild-caught Atlantic salmon fillet pan-seared in ghee, served alongside garlic asparagus.", tags: ["460 KCAL", "OMEGA 3"], btnLabel: "Prepare Dinner", icon: "plus" }
    ]);
    setRecommendedSwaps([
      { _id: "swap-1", tag: "LOWER CARB", tagIcon: "zap", title: "Zucchini Noodle Stir-fry", desc: "Swap standard noodles for spiralized zucchini.", swapLabel: "Swap Lunch", stars: 2 },
      { _id: "swap-2", tag: "HEART HEALTHY", tagIcon: "heart", title: "Avocado Tofu Scramble", desc: "Substitute eggs with turmeric-spiced tofu.", swapLabel: "Swap Breakfast", stars: 1 }
    ]);
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#F4EFE6] flex items-center justify-center font-bold text-[#1E3D30]">
        Loading custom nutritional plans...
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-screen bg-[#F4EFE6]" style={{ fontFamily: "'DM Sans', 'Nunito', sans-serif" }}>
        ...
        <aside className={`fixed top-0 left-0 h-full z-40 w-56 bg-white flex flex-col py-7 px-4 shadow-xl transition-transform duration-300 ease-in-out ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 lg:static lg:shadow-none lg:z-auto`}>
          ...
          <nav className="space-y-1.5 flex-1">
            {NAV.map(item => {
              const active = location.pathname === item.path;
              return (
                <Link key={item.label} to={item.path} className={`flex items-center gap-3.5 px-4 py-3 rounded-2xl text-sm font-bold tracking-tight transition-all duration-200 ${active ? "bg-[#1E3D30] text-white shadow-md shadow-green-900/10" : "text-gray-400 hover:bg-gray-50 hover:text-[#1A1A1A]"}`}>
                  <Icon d={Icons[item.icon]} className={active ? "text-[#E8693A]" : "text-gray-400"} />
                  <span>{item.label}</span>
                </Link>
              );
      ...
      <div className="flex-1 min-w-0 flex flex-col h-screen overflow-y-auto">
        <header className="px-4 lg:px-10 py-5 flex items-center justify-between bg-[#F4EFE6]/80 backdrop-blur sticky top-0 z-20">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(true)} className="lg:hidden w-11 h-11 rounded-xl bg-white flex items-center justify-center border border-gray-200 shadow-sm text-gray-600">
              <Icon d={Icons.menu} size={20} />
            </button>
            <div>
              <h1 className="font-black text-[#1A1A1A] text-xl lg:text-2xl tracking-tight">Weekly Meal Plan</h1>
        ...
        <main className="px-4 lg:px-10 pb-12 grid grid-cols-1 xl:grid-cols-3 gap-8 max-w-[1400px] w-full mx-auto">
          <div className="xl:col-span-2 space-y-8">
            ...
            {/* Primary Featured Display (Read) */}
            {featuredMeal && (
              <div className="bg-white rounded-3xl overflow-hidden shadow-sm grid grid-cols-1 md:grid-cols-12 group">
                <div className="relative h-64 md:h-auto md:col-span-5 overflow-hidden">
                  <img src={featuredMeal.img || "https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=800"} alt={featuredMeal.title} className="w-full h-full object-cover group-hover:scale-103 transition-transform duration-700" />
                  <MealBadge label={featuredMeal.badge || "LUNCH (TODAY)"} dark />
                </div>
                <div className="p-6 lg:p-8 md:col-span-7 flex flex-col justify-between">
                  <div>
                    <div className="flex gap-2 mb-3">
                      {featuredMeal.tags?.map(t => <Tag key={t}>{t}</Tag>)}
                    </div>
                    <h2 className="font-black text-[#1A1A1A] text-2xl mb-2 tracking-tight">{featuredMeal.title}</h2>
                    <p className="text-gray-400 text-sm leading-relaxed mb-6">{featuredMeal.desc}</p>
                  </div>
                  <div className="flex flex-wrap items-center gap-4 pt-4 border-t border-gray-100">
                    <button onClick={() => handleLogMealItem(featuredMeal._id)} className="bg-[#1E3D30] text-white text-sm font-bold px-6 py-3.5 rounded-xl hover:bg-[#162e23] active:scale-95 transition-all duration-150 shadow-sm">
                      Log Featured Recipe
                    </button>
                  </div>
                </div>
            ...
            {/* Supplementary Standard Meals Grid Mapping */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {complementaryMeals.map((meal) => (
                <SmallMealCard 
                  key={meal._id}
                  id={meal._id}
                  badge={meal.badge || "PLAN"}
                  img={meal.img}
                  title={meal.title}
                  desc={meal.desc}
                  tags={meal.tags || []}
                  btnLabel={meal.btnLabel || "Log Meal"}
                  icon={meal.icon || "plus"}
                  onAction={handleLogMealItem}
                />
              ))}
            </div>
          </div>

          {/* Right Info Column Widget Layout */}
          <div className="space-y-8">
            <div className="bg-[#1E3D30] rounded-3xl p-6 lg:p-8 text-white shadow-xl flex items-center justify-between gap-6 relative overflow-hidden">
              <div className="space-y-2">
                <span className="text-[10px] font-black tracking-widest text-[#E8693A] block uppercase">Macros Blueprint</span>
                ...
                <p className="text-white/60 text-xs leading-relaxed max-w-[160px]">
                  You are {nutritionalSummary.current.toLocaleString()} kcal deep into your {nutritionalSummary.target.toLocaleString()} limit setup.
                </p>
              </div>
              <CircularProgress pct={progressPercentage} />
            </div>

            {/* Smart Swap Core Recommendations with Action Hooks */}
            <div className="bg-white rounded-3xl p-6 lg:p-8 shadow-sm space-y-5">
              <h3 className="font-black text-[#1A1A1A] text-lg tracking-tight">Smart Swap Alternatives</h3>
              <div className="divide-y divide-gray-100 space-y-4">
                {recommendedSwaps.map((card, idx) => (
                  <div key={card._id || idx} className="pt-4 first:pt-0 flex flex-col items-start relative group">
                    <button 
                      onClick={() => handleRemoveMealItem(card._id)}
                      className="absolute right-0 top-4 opacity-0 group-hover:opacity-100 text-gray-300 hover:text-red-500 transition-opacity"
                      title="Dismiss Alternative Choice"
                    >
                    ...
                    <div className="flex items-center justify-between w-full mb-2">
                      <div className="w-6 h-6 rounded-lg bg-[#F4EFE6] flex items-center justify-center">
                        <Icon d={Icons[card.tagIcon || "zap"]} size={12} className="text-[#1E3D30]" />
                      </div>
                      <span className="bg-[#1E3D30] text-white text-[10px] font-black tracking-widest px-2.5 py-1 rounded-full">
                        {card.tag || "HEALTHY"}
                      </span>
                    ...
                    <button 
                      onClick={() => handleLogMealItem(card._id)}
                      className="text-xs font-bold text-[#E8693A] hover:text-[#1E3D30] transition-colors"
                    >
                      ★ {card.swapLabel || "Swap into Plan"}
                    </button>
                  </div>
                ))}
              </div>
