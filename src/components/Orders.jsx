...
// ─── Order card ───────────────────────────────────────────────────────────────
function OrderCard({ order, expanded, onToggle, onCancelOrder }) {
  return (
    <div className={`bg-white rounded-2xl border border-gray-100 shadow-sm transition-shadow hover:shadow-md overflow-hidden`}>
      {/* Header row */}
      <div className="flex items-start sm:items-center justify-between gap-3 p-4 sm:p-5 cursor-pointer select-none" onClick={onToggle}>
        <div className="flex items-start sm:items-center gap-3 flex-1 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-0.5">
              <span className="font-extrabold text-[#1a2e2a] text-sm">{order.orderNumber || order.id}</span>
              {(order.tags || ["Prescription"]).map(t => (
                <span key={t} className="text-[9px] font-bold uppercase tracking-widest bg-[#f0ede8] text-[#c96a4f] px-2 py-0.5 rounded-full">{t}</span>
              ))}
            </div>
            <p className="text-gray-400 text-xs truncate">{order.pharmacy || "Main Pharmacy"} · {new Date(order.createdAt || order.date).toLocaleDateString()}</p>
          </div>
        </div>
        <div className="flex items-center gap-3 flex-shrink-0">
          <div className="hidden sm:flex flex-col items-end gap-1">
            <StatusBadge status={order.status} />
            <span className="text-xs text-gray-400">{order.eta || "Standard Delivery"}</span>
          </div>
          <span className="font-extrabold text-[#1a2e2a] text-sm">${order.totalAmount || order.total}</span>
          <Icon d={IC.chevDown} size={15} className={`text-gray-400 transition-transform duration-200 ${expanded ? "rotate-180" : ""}`} />
        </div>
      </div>
        ...
        <div className="border-t border-gray-100 px-5 py-4 space-y-4 animate-fadeIn">
          <div>
            ...
            <ul className="space-y-1.5">
              {(order.items || []).map((item, i) => (
                <li key={i} className="flex items-center gap-2 text-sm text-[#1a2e2a]">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#c96a4f] flex-shrink-0" />
                  {typeof item === 'string' ? item : `${item.name || "Medical Item"} × ${item.quantity || 1}`}
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pt-1">
            {order.rating ? (
              ...
              /* USES ENDPOINT: PATCH /orders/{id}/cancel */
              /* THIS BUTTON TRIGGERS THE CANCEL ORDER ENDPOINT INTEGRATED ON THE PARENT LAYOUT */
              <button 
                onClick={(e) => { e.stopPropagation(); onCancelOrder(order._id || order.id); }}
                className="text-xs font-bold text-red-500 hover:underline flex items-center gap-1"
              >
                <Icon d={IC.xCircle} size={12} className="text-red-500" /> Cancel Order
              </button>
            ) : <span />}
...
export default function Orders() {
  ...
  // State hooks for unified server data
  const [ordersList, setOrdersList] = useState([]);
  const [healthProfile, setHealthProfile] = useState(null);
  const [loading, setLoading] = useState(true);

  /* USES ENDPOINT: GET /orders */
  /* FETCHES SYSTEM-WIDE ACCOUNT ORDERS ASSIGNED TO CURRENTLY AUTHENTICATED ACCESS ENTITY */
  useEffect(() => {
  ...
  /* USES ENDPOINT: GET /api/v1/health-profiles/me */
  /* GRABS CURRENT HEALTH PROFILE SPECIFIC TO APPLICANT FOR FILLING ANALYTICS & DASHBOARD PROFILES */
  useEffect(() => {
  ...
  /* USES ENDPOINT: PATCH /orders/{id}/cancel */
  /* SENDS MODIFICATION TO UPDATE THE SPECIFIC LIFECYCLE STATE ATTRIBUTE OF AN ORDER TO 'CANCELLED' */
  ...
  // Compute stats on fly from active operational dataset state
  const totalSpentCalculated = ordersList
    .filter(o => o.status !== "Cancelled")
    .reduce((acc, curr) => acc + (Number(curr.totalAmount || curr.total) || 0), 0);

  const SUMMARY_CARDS = [
    { label: "Total Orders",    value: ordersList.length.toString(),     sub: "Lifetime count",   icon: IC.package,  color: "bg-[#f0ede8]", iconColor: "text-[#c96a4f]" },
    { label: "Active Refills",  value: "3",      sub: "Auto-ship enabled", icon: IC.repeat,   color: "bg-[#e8f5f0]", iconColor: "text-[#3a7d5a]" },
    { label: "Avg. Delivery",   value: "3.2d",  sub: "Last 90 days",     icon: IC.truck,    color: "bg-[#e8f0ef]", iconColor: "text-[#1a6a7e]" },
    { label: "Total Spent",     value: `$${totalSpentCalculated.toFixed(2)}`,  sub: "Non-cancelled",        icon: IC.leaf,     color: "bg-[#fdf3e8]", iconColor: "text-[#b87333]" },
  ];

  const filtered = ordersList.filter(o => {
    const matchFilter = activeFilter === "All" || o.status === activeFilter;
    const searchString = searchQ.toLowerCase();
    const matchSearch = searchQ === "" || 
      (o.orderNumber || o.id || "").toLowerCase().includes(searchString) ||
      (o.pharmacy || "").toLowerCase().includes(searchString);
    return matchFilter && matchSearch;
  });
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#f5f3ef] flex items-center justify-center font-sans">
        <p className="text-[#1a2e2a] font-extrabold text-sm animate-pulse">Syncing order profiles...</p>
      </div>
    );
  }
        ...
        <main className="flex-1 overflow-y-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
          ...
          <div className="flex flex-col xl:flex-row gap-6">
                ...
                {filtered.length === 0 ? (
                  ...
                  filtered.map(order => (
                    <OrderCard
                      ...
                      order={order}
                      expanded={expandedId === (order._id || order.id)}
                      onToggle={() => setExpandedId(expandedId === (order._id || order.id) ? null : (order._id || order.id))}
            ...
            <div className="w-full xl:w-72 flex-shrink-0 space-y-4">
              ...
              {/* Dynamic Address block filled from health profile backend sync */}
              <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
                <div className="flex items-center justify-between mb-3">
                  <h2 className="font-extrabold text-[#1a2e2a] text-sm">Default Address</h2>
                  <button className="text-[11px] font-bold text-[#c96a4f] hover:underline">Edit</button>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    {/* Populates default fallback structure or actual parsed server health profile names */}
                    <p className="text-sm font-bold text-[#1a2e2a]">{healthProfile?.fullName || "Sarah M."}</p>
                    <p className="text-xs text-gray-400 leading-relaxed mt-0.5">
                      {healthProfile?.address || "14 Wellspring Crescent"}<br />
                      {healthProfile?.cityStatePin || "Suite 3B, Melbourne VIC 3000"}
                    </p>
                  </div>
                </div>
              </div>
